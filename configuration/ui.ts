import { AI_NAME, OWNER_NAME } from "@/configuration/identity";

export const CHAT_HEADER: string = `Your Personalized Running Coach`;
export const MESSAGE_PLACEHOLDER: string = `Ask me anything about training, injuries, or running motivation!`;
export const FOOTER_MESSAGE: string = `Keep moving forward—every step counts!`;
export const CLEAR_BUTTON_TEXT: string = `Clear Chat`;
export const PAGE_TITLE: string = `RunSmart Buddy – Your AI Running Coach`;
export const PAGE_DESCRIPTION: string = `Chat with ${AI_NAME}, ${OWNER_NAME}'s AI assistant, for smarter running, better performance, and injury-free training.`;

export const EMPTY_CITATION_MESSAGE: string = "Unspecified source";
